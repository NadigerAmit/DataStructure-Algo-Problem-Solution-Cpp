...
class WorkingStudent : public Employe,public Student {
	...
	public:
	    WorkingStudent(int age ,string name,int stdId):
   	        Person(age,name),Employe(age,name),Student(age,stdId,name),workingStudentId(11){;}  // here should explicitly pass the parameters to base class constructor i.e Person(age,name)
...
In other words, the WorkingStudent class will have only a single instance of the Person class, shared by both the Student and Employe classes. By having a single instance of Person, 
...
In order to keep track of the single instance of the Person object, the compiler will provide a virtual function table (vtable) for classes Student and Employe. 
...
When the Student class or the Employe class goes to access any fields of the Person, it uses the virtual pointer in its vtable to find the Person object and find the field in it.
...
the constructor for a virtual base class is not called by the class that inherits from it (which is how constructors are called, when each class has its own copy of its parent class) 
since that would mean the constructor would run multiple times. Instead, the constructor is called by the constructor of the concrete class. 
...
One thing to be aware of is that if either Student or Employe attempted to invoke the Person constructor in their initialization lists, that call will be completely skipped 
...
the destructors run in the opposite order of the constructors. In other words, the virtual base class will be the last object destroyed, because it is the first object that is fully constructed.
*/